...
from aml_rule_engine import get_aml_rule_queries
...
SUPPORTED_RULE_CATEGORIES = {
    ...
    "Schema Design": [
        "Optional AML rule catalog design",
...
ORDER BY p.WEIGHT_IN_FUND_PCT DESC NULLS LAST""",
    ...
    "aml_alerts": {
        "finding": "Building a live AML alert-style inventory directly from the AML rule engine scenarios over the current source tables. This gives a consolidated review queue without requiring FACT_AML_ALERT persistence.",
        "regulation": "AML Monitoring — Live Rule-Engine Alert Inventory",
        "severity": "HIGH",
        "rule_key": "aml_alerts",
        "category": "AML Monitoring",
        "sql": """SELECT *
FROM (
    SELECT
        CONCAT('AML-STRUCT-', da.ACCOUNT_ID, '-', TO_VARCHAR(dd.CALENDAR_DATE, 'YYYYMMDD')) AS ALERT_ID,
        'Potential Structuring Activity' AS ALERT_TITLE,
        'OPEN' AS ALERT_STATUS,
        'HIGH' AS ALERT_SEVERITY,
        SUM(ABS(ft.GROSS_AMOUNT)) AS ALERT_SCORE,
        dd.CALENDAR_DATE AS EVENT_DATE,
        ft.ACCOUNT_KEY,
        MAX(ft.COUNTERPARTY_KEY) AS COUNTERPARTY_KEY,
        MAX(ft.FUND_KEY) AS FUND_KEY,
        'AML_TIER2' AS ASSIGNED_ANALYST,
        NULL AS CASE_ID,
        NULL AS DISPOSITION,
        FALSE AS SAR_FILED_FLAG,
        MAX(ft.TRADE_TIMESTAMP) AS ALERT_CREATED_AT,
        NULL AS CLOSED_AT,
        'aml_structuring' AS SOURCE_RULE_KEY
    FROM REGULATORY_DW.REG_MODEL.FACT_TRANSACTION ft
    ...
    GROUP BY da.ACCOUNT_ID, ft.ACCOUNT_KEY, dd.CALENDAR_DATE
    HAVING COUNT(*) >= 3
    ...
    SELECT
        CONCAT('AML-VELOCITY-', da.ACCOUNT_ID, '-', TO_VARCHAR(TO_DATE(DATE_TRUNC('day', ft.TRADE_TIMESTAMP)), 'YYYYMMDD')) AS ALERT_ID,
        'Unusual Transaction Velocity' AS ALERT_TITLE,
        'OPEN' AS ALERT_STATUS,
        'MEDIUM' AS ALERT_SEVERITY,
        SUM(ABS(ft.GROSS_AMOUNT)) AS ALERT_SCORE,
        TO_DATE(DATE_TRUNC('day', ft.TRADE_TIMESTAMP)) AS EVENT_DATE,
        ft.ACCOUNT_KEY,
        MAX(ft.COUNTERPARTY_KEY) AS COUNTERPARTY_KEY,
        MAX(ft.FUND_KEY) AS FUND_KEY,
        'AML_TIER1' AS ASSIGNED_ANALYST,
        NULL AS CASE_ID,
        NULL AS DISPOSITION,
        FALSE AS SAR_FILED_FLAG,
        MAX(ft.TRADE_TIMESTAMP) AS ALERT_CREATED_AT,
        NULL AS CLOSED_AT,
        'aml_velocity' AS SOURCE_RULE_KEY
    FROM REGULATORY_DW.REG_MODEL.FACT_TRANSACTION ft
    JOIN REGULATORY_DW.REG_MODEL.DIM_ACCOUNT da ON ft.ACCOUNT_KEY = da.ACCOUNT_KEY
    WHERE da.IS_CURRENT = TRUE
    GROUP BY da.ACCOUNT_ID, ft.ACCOUNT_KEY, DATE_TRUNC('day', ft.TRADE_TIMESTAMP)
    HAVING COUNT(*) >= 10
    ...
    SELECT
        CONCAT('AML-ROUND-', ft.TRANSACTION_ID) AS ALERT_ID,
        'Suspicious Round-Amount Transaction' AS ALERT_TITLE,
        'OPEN' AS ALERT_STATUS,
        'MEDIUM' AS ALERT_SEVERITY,
        ABS(ft.GROSS_AMOUNT) AS ALERT_SCORE,
        TO_DATE(ft.TRADE_TIMESTAMP) AS EVENT_DATE,
        ft.ACCOUNT_KEY,
        ft.COUNTERPARTY_KEY,
        ft.FUND_KEY,
        'AML_TIER1' AS ASSIGNED_ANALYST,
        ...
        NULL AS DISPOSITION,
        FALSE AS SAR_FILED_FLAG,
        ft.TRADE_TIMESTAMP AS ALERT_CREATED_AT,
        NULL AS CLOSED_AT,
        'aml_round_amount' AS SOURCE_RULE_KEY
    FROM REGULATORY_DW.REG_MODEL.FACT_TRANSACTION ft
    JOIN REGULATORY_DW.REG_MODEL.DIM_ACCOUNT da ON ft.ACCOUNT_KEY = da.ACCOUNT_KEY
    WHERE da.IS_CURRENT = TRUE
    ...
    SELECT
        CONCAT('AML-DORMANT-', da.ACCOUNT_ID, '-', rlt.TRANSACTION_ID) AS ALERT_ID,
        'Dormant Account Reactivation' AS ALERT_TITLE,
        'OPEN' AS ALERT_STATUS,
        'HIGH' AS ALERT_SEVERITY,
        ABS(rlt.GROSS_AMOUNT) AS ALERT_SCORE,
        rlt.CALENDAR_DATE AS EVENT_DATE,
        rlt.ACCOUNT_KEY,
        ft.COUNTERPARTY_KEY,
        ft.FUND_KEY,
        'AML_TIER2' AS ASSIGNED_ANALYST,
        ...
        ft.TRADE_TIMESTAMP AS ALERT_CREATED_AT,
        NULL AS CLOSED_AT,
        'aml_dormancy' AS SOURCE_RULE_KEY
    FROM (
        SELECT ft.ACCOUNT_KEY, ft.TRANSACTION_ID, dd.CALENDAR_DATE, ft.GROSS_AMOUNT
        FROM REGULATORY_DW.REG_MODEL.FACT_TRANSACTION ft
        JOIN REGULATORY_DW.REG_MODEL.DIM_DATE dd ON ft.TRADE_DATE_KEY = dd.DATE_KEY
    ...
    JOIN REGULATORY_DW.REG_MODEL.FACT_TRANSACTION ft ON ft.TRANSACTION_ID = rlt.TRANSACTION_ID
    ...
    SELECT
        CONCAT('AML-SANCTIONS-', cp.COUNTERPARTY_KEY) AS ALERT_ID,
        'Sanctions Screening Exception' AS ALERT_TITLE,
        'OPEN' AS ALERT_STATUS,
        'HIGH' AS ALERT_SEVERITY,
        DATEDIFF('day', cp.SANCTIONS_SCREENED_DATE, CURRENT_DATE()) AS ALERT_SCORE,
        CURRENT_DATE() AS EVENT_DATE,
        NULL AS ACCOUNT_KEY,
        cp.COUNTERPARTY_KEY,
        NULL AS FUND_KEY,
        'SANCTIONS_OPS' AS ASSIGNED_ANALYST,
        NULL AS CASE_ID,
        NULL AS DISPOSITION,
        FALSE AS SAR_FILED_FLAG,
        CURRENT_TIMESTAMP() AS ALERT_CREATED_AT,
        NULL AS CLOSED_AT,
        'aml_sanctions' AS SOURCE_RULE_KEY
...
ORDER BY ALERT_CREATED_AT DESC, ALERT_SEVERITY DESC, ALERT_SCORE DESC""",
        "remediation": "Use this live queue for demo-time review of active AML scenarios. If you later need case ownership, dispositions, and closed-alert history, persist these results into FACT_AML_ALERT.",
        "audit_note": "Live AML alert inventory generated from rule-engine scenarios"
    },
    "breach_explain": {
...
WHERE BREACH_ID = '<BREACH_ID>'""",
    ...
    "suitability_exceptions": {
        "finding": "Checking for client suitability exceptions directly from current account, transaction, and product data. This produces a live suitability exception view without requiring the persisted suitability assessment table.",
        "regulation": "MiFID II / Suitability Governance — Client Appropriateness and Suitability",
        "severity": "HIGH",
        "rule_key": "suitability_exceptions",
        "category": "Regulatory Reporting",
        "sql": """SELECT
    CONCAT('SUIT-', da.ACCOUNT_ID, '-', s.SECURITY_ID) AS ASSESSMENT_ID,
    da.ACCOUNT_ID,
    da.ACCOUNT_NAME,
    s.SECURITY_ID AS PRODUCT_ID,
    s.SECURITY_NAME AS PRODUCT_NAME,
    COALESCE(s.ESG_RATING, 'MEDIUM') AS PRODUCT_RISK_RATING,
    COALESCE(da.RISK_PROFILE, 'MEDIUM') AS CLIENT_RISK_TOLERANCE,
    CASE
        WHEN da.CLIENT_CLASSIFICATION = 'PENSION_FUND' THEN 'INCOME_ONLY'
        ELSE 'STANDARD'
    END AS MANDATE_RESTRICTION,
    CASE
        WHEN COALESCE(da.RISK_PROFILE, 'MEDIUM') = 'LOW' AND s.ASSET_CLASS = 'DERIVATIVE' THEN 'EXCEPTION'
        ELSE 'SUITABLE'
    END AS SUITABILITY_STATUS,
    CASE
        WHEN COALESCE(da.RISK_PROFILE, 'MEDIUM') = 'LOW' AND s.ASSET_CLASS = 'DERIVATIVE' THEN 'Low-risk mandate matched to derivative exposure'
        ELSE NULL
    END AS EXCEPTION_REASON,
    CURRENT_TIMESTAMP() AS REVIEWED_AT,
    CASE
        WHEN COALESCE(da.RISK_PROFILE, 'MEDIUM') = 'LOW' AND s.ASSET_CLASS = 'DERIVATIVE' THEN TRUE
        ELSE FALSE
    END AS REMEDIATION_REQUIRED
FROM REGULATORY_DW.REG_MODEL.FACT_TRANSACTION ft
JOIN REGULATORY_DW.REG_MODEL.DIM_ACCOUNT da ON ft.ACCOUNT_KEY = da.ACCOUNT_KEY
JOIN REGULATORY_DW.REG_MODEL.DIM_SECURITY s ON ft.SECURITY_KEY = s.SECURITY_KEY
WHERE da.IS_CURRENT = TRUE
  AND s.IS_CURRENT = TRUE
  AND COALESCE(da.RISK_PROFILE, 'MEDIUM') = 'LOW'
  AND s.ASSET_CLASS = 'DERIVATIVE'
GROUP BY
    da.ACCOUNT_ID,
    da.ACCOUNT_NAME,
    s.SECURITY_ID,
    s.SECURITY_NAME,
    COALESCE(s.ESG_RATING, 'MEDIUM'),
    COALESCE(da.RISK_PROFILE, 'MEDIUM'),
    da.CLIENT_CLASSIFICATION,
    s.ASSET_CLASS
ORDER BY REVIEWED_AT DESC, da.ACCOUNT_NAME, s.SECURITY_NAME""",
        ...
        "audit_note": "Live suitability exception query prepared"
    },
    "transaction_audit_trail": {
...
AML_RULE_BASED_QUERIES = get_aml_rule_queries()


AML_SCHEMA_RULE_BASED_QUERIES = {
    "aml_rule_schema": {
        "finding": "Designing a database-side rule catalog table for AML detections when configuration must be managed in Snowflake rather than in the Python AML rule engine. In the current demo architecture this is optional legacy design, not the primary source of truth.",
    ...
    CONSTRAINT UQ_DIM_AML_RULE_RULE_ID_VERSION UNIQUE (RULE_ID, VERSION_NUMBER)
        ...
        "remediation": "Use the Python AML rule library as the primary source of truth for the demo. Only create this table if you later need Snowflake-managed rule administration.",
    ...
    SOURCE_RULE_KEY VARCHAR(100) NOT NULL,
    ...
    CONSTRAINT UQ_FACT_AML_ALERT_ALERT_ID UNIQUE (ALERT_ID)
        ...
        "remediation": "Populate the alert table from the Python AML rule engine outputs and link alerts into case management using CASE_ID, analyst assignment, and disposition fields.",
...
def build_planned_rule_response(rule_key):
    ...
    return {
        "finding": f"This request is recognized, but {display_name} it is not available in live mode yet and is planned for PHASE-II release.",
